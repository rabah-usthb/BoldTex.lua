...
function bold.fetchSelectedText(start_line,start_col,end_line,end_col)
 ...
 -- print("Lines: " .. table.concat(lines, "\n"))  -- Use table.concat to join the lines
  ...
  if start_line == end_line  then
    selectedLines[1] = "textbf{"..string.sub(lines[1],start_col,end_col)
  else
      for i = 1, #lines, 1 do
       if i == 1 then
         selectedLines[i] = string.sub(lines[1],start_col,#lines[1])
           elseif i == (end_line - start_line + 1) then
          selectedLines[i] = string.sub(lines[i],0,end_col)..'}'
            else
